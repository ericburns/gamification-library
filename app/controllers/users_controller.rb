class UsersController < ApplicationController

  layout "application_with_admin_panel"
  
  # GET /users
  # GET /users.json
  #
  # before_filter :save_login_state, :only => [:new, :create]

  before_filter :get_game, :only => [:show, :edit, :update, :destroy, :index]

  def index
    @users = @game.users

    respond_to do |format|
      format.html # index.html.erb
      format.json { render json: @users }
    end
  end

  # GET /users/1
  # GET /users/1.json
  def show
    @user = @game.users.find(params[:id])

    respond_to do |format|
      format.html # show.html.erb
      format.json { render json: @user }
    end
  end

  # GET /users/new
  # GET /users/new.json
  def new
    @user = User.new

    respond_to do |format|
      format.html # new.html.erb
      format.json { render json: @user }
    end
  end

  # GET /users/1/edit
  def edit
    @user = User.find(params[:id])
  end

  # POST /users
  # POST /users.json
  def create
    @user = @game.users.new(params[:user])

    respond_to do |format|
      if @user.save
        format.html { redirect_to [@game, @user], notice: 'User was successfully created.' }
        format.json { render json: [@game, @user], status: :created, location: [@game, @user] }
      else
        format.html { render action: "new" }
        format.json { render json: @user.errors, status: :unprocessable_entity }
      end
    end
  end

  # PUT /users/1
  # PUT /users/1.json
  def update
    @user = @game.users.find(params[:id])

    respond_to do |format|
      if @user.update_attributes(params[:user])
        format.html { redirect_to [@game, @user], notice: 'User was successfully updated.' }
        format.json { head :no_content }
      else
        format.html { render action: "edit" }
        format.json { render json: @user.errors, status: :unprocessable_entity }
      end
    end
  end

  # DELETE /users/1
  # DELETE /users/1.json
  def destroy
    @user = @game.users.find(params[:id])
    @user.destroy

    respond_to do |format|
      format.html { redirect_to game_users_url }
      format.json { head :no_content }
    end
  end

  # Used for getting the game each user is associated with.
  def get_game
    @game = Game.find(params[:game_id])
  end
end
